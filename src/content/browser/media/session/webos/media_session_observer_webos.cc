...
MediaSessionObserverWebOS::MediaSessionObserverWebOS(MediaSessionImpl* session)
  ...
  application_id_ = GetAppIdOfWebContents();
...
void MediaSessionObserverWebOS::SetPlaybackStatusInternal(
  ...
  auto get_playback_status = [&](MediaPlaybackState staus) {
    auto it = kPlaybackStateMap.find(staus);
...
void MediaSessionObserverWebOS::HandleMediaKeyEvent(
  ...
  if (!response->GetString(kMediaId, &session_id) || session_id.empty()) {
    LOG(WARNING) << __func__ << " Session ID is not sent";
    return;
  }

  if (session_id_ != session_id) {
    LOG(INFO) << __func__ << " Event recieved for other session. Ignore.";
    return;
  }
...
std::string MediaSessionObserverWebOS::GetAppIdOfWebContents() {
  if (!media_session_)
    return std::string();
...
void MediaSessionObserverWebOS::HandleMediaKeyEventInternal(
  ...
  LOG(INFO) << __func__ << " key_event: " << key_event;
  ...
  static std::map<std::string, MediaKeyEvent> kEventKeyMap = {
      ...
      {kMuteEvent, MediaSessionObserverWebOS::MediaKeyEvent::kMute},
      {kUnmuteEvent, MediaSessionObserverWebOS::MediaKeyEvent::kUnmute}};
  ...
  if (media_session_) {
    ...
    switch (event_type) {
      ...
      case MediaSessionObserverWebOS::MediaKeyEvent::kPrevious:
        ...
        break;
      case MediaSessionObserverWebOS::MediaKeyEvent::kMute:
        media_session_->SetMuted(true);
        break;
      case MediaSessionObserverWebOS::MediaKeyEvent::kUnmute:
        media_session_->SetMuted(false);
        break;
      default:
